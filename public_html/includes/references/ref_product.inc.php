      ...
      switch($field) {
        ...
        case 'also_purchased_products':
          ...
          $this->_data['also_purchased_products'] = array();
            ...
            $query = database::query(
              ...
              where (product_id != '". database::input($this->_id) ."' and product_id not like '". database::input($this->_id) .":%')
              and order_id in (
                select distinct order_id as id from ". DB_TABLE_ORDERS_ITEMS ."
                where (product_id = '". database::input($this->_id) ."' or product_id like '". database::input($this->_id) .":%')
              )
              group by product_id
        ...
        case 'attributes':
          ...
          $query = database::query(
            ...
            where product_id = '". database::input($this->_id) ."'
        ...
        case 'campaign':
          ...
          if ($products_campaign[$this->_currency_code] > 0) {
            $this->_data['campaign']['price'] = currency::convert($products_campaign[$this->_currency_code], $this->_currency_code, settings::get('store_currency_code'));
          } else {
            $this->_data['campaign']['price'] = $products_campaign[settings::get('store_currency_code')];
          }
